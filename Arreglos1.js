...
const b = [1, 2, 8, 4]; //Arreglo con 4 elementos
...
for (let i = 0; i < pelicula.length; i++) {
  console.log(`Película: ${i} Nombre: ${pelicula[i]}`);
}
...
const peliculas2 = ["La Milla Verde", "Parásitos", "El Viaje de Chihiro", "Coco", "Interstellar"];

peliculas2.forEach((peli, index) => console.log(`Pelicula Posición #${index} : ${peli}`));

let pan=["La milla verde" ,"Parásitos","El viaje de Chihiro","Coco" , "Interstellar"];

pan.map((pan) => {
  console.log(`pelicula ${pan}`);
});

pan.map((pan,index)=>{
    console.log(`pelicula ${index} ${pan}`)
    })
...
for (let i = 0; i < peliculas.length; i++){
  console.log("Película ", i, ":", peliculas3[i]);
}